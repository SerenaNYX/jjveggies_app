...
<div class="container">
    <div>
        <a href="{{ route(auth('employee')->user()->role . '.orders.index') }}" class="btn back-btn" style="margin-bottom:1rem;">&larr;</a>
        <div class="card">
            <div class="card-header">
                <h2 style="color: black;">#{{ $order->order_number }}</h2>
                <span class="badge 
            ...
            <div class="card-body">
                <div class="">
                    ...
                    <p>UID: {{ $order->user->uid }}<br>
                    Name: {{ $order->user->name }}<br>
                    Email: {{ $order->user->email }}<br>
                    Phone: {{ $order->user->contact ?? 'N/A' }}</p>
                </div>
                <hr>
                <div class="">
                ...
                @if(auth('employee')->user()->role === 'staff' || auth('employee')->user()->role === 'driver')
                    <h3 style="margin-bottom: 8px;">Update Status</h3>
                    @php
